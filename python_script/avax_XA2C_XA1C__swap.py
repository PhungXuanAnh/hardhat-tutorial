import json
from avax_constant import (
    w3, PANGOLIN_ROUTER_V2_ADDRESS, PANGOLIN_FACTORY_ADDRESS,
...
router_contract = w3.eth.contract(address=PANGOLIN_ROUTER_V2_ADDRESS, abi=router_abi)
...
XA1C_allowance = XA1C.functions.allowance(ACCOUNT_2_ADDRESS, PANGOLIN_ROUTER_V2_ADDRESS).call()
print("account_2 have gave router {} XA1C ".format(XA1C_allowance / 10**2))

XA2C_allowance = XA2C.functions.allowance(ACCOUNT_2_ADDRESS, PANGOLIN_ROUTER_V2_ADDRESS).call()
...
print('amounts out [XA1C, XA2C] = {}'.format(amounts_out))
    
# get current balance of account 2

old_XA1C_balance = XA1C.functions.balanceOf(ACCOUNT_2_ADDRESS).call() / 10**2
old_XA2C_balance = XA2C.functions.balanceOf(ACCOUNT_2_ADDRESS).call() / 10**2
...
print('new balance XA1C of account 2: ', XA1C.functions.balanceOf(ACCOUNT_2_ADDRESS).call() / 10**2)
print('new balance XA2C of account 2: ', XA2C.functions.balanceOf(ACCOUNT_2_ADDRESS).call() / 10**2)
