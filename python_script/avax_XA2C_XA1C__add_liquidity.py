...
from avax_constant import (
    ...
    XA1C_ADDRESS, XA2C_ADDRESS, LIFE_ADDRESS,
...
XA1C_allowance = XA1C.functions.allowance(OPERATOR_ADDRESS, PANGOLIN_ROUTER_V2_ADDRESS).call()
print("Operator have gave router {} XA1C ".format(XA1C_allowance / 10**2))

XA2C_allowance = XA2C.functions.allowance(OPERATOR_ADDRESS, PANGOLIN_ROUTER_V2_ADDRESS).call()
...
if XA1C_allowance < amount_XA1C_desired:
    ...
    print('-------- approved to router spend amount of XA1C')
...
if XA2C_allowance < amount_XA2C_desired:
    ...
    txn_receipt = w3.eth.wait_for_transaction_receipt(txn_hash)
    print(json.dumps(txn_receipt, indent=4, sort_keys=True, cls=BlockchainDataJsonEncoder))
    print('-------- approved to router spend amount of XA2C')
